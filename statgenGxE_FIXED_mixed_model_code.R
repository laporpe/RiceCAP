...
pheno <- pheno %>% filter(DaysHEAD > 0) %>% filter(DaysGrainfill < 150)
...
## Check if the data contains replicates.
  ...
  ggplot(long1, aes(fill=var1, y=value, x=trait, label=ifelse(value>0.001, round(value,digits = 3),""))) + 
    geom_bar(position="fill", stat="identity") + theme_bw() + ylab("Percent Variance Explained") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + labs(fill = "")+
    scale_x_discrete(labels=c("MeanHT" = "Plant Height",
                       ...
                       geom_text(size = 3, position = position_stack(vjust = 0.5)) + theme(axis.text = element_text(size = 15)) +theme(axis.title = element_text(size = 15))
  
  
  ggsave("/Users/maryfrancis/Documents/Diepenbrock_Lab/rice-cgm-wgp/figures/GxE.tiff", units="in", width=10, height=10, dpi=300, compression = 'lzw')
